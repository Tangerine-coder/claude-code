...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      (uniforms.iResolution.value as [number, number, number]) = [gl.drawingBufferWidth, gl.drawingBufferHeight, 1];
    };
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (mouseDampening <= 0) {
        (uniforms.iMouse.value as [number, number]) = [x, y];
      }
    };
    if (mouseInteraction) {
    ...
    dpr, paused, colors.join(','), backgroundColor, speed, streakCount, streakWidth, streakLength,
  ...
  return (
    <div
      ...
      style={mixBlendMode ? ({ mixBlendMode } as React.CSSProperties) : undefined}
    />
  );
}
